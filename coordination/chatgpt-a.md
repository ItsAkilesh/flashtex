...
- Evidence: eleven harness unit tests pass. The repaired candidate is `d764d54` on `agent/chatgpt-a/companion-reliability`; its unsigned Debug app and XCTest-target builds passed on Xcode 26.6. The owner branch `e7ce5b94a12e372a482a9578ac9e121b09c51c8a` remains malformed. A new cross-transport gate reproduces a real disconnected-path regression: `CaptureTransport` prints the payload, then `BonjourTransport` prints that same payload through its stdout fallback. This Mac has no installed iOS simulator runtime, so XCTest execution is explicitly pending and is not claimed.
