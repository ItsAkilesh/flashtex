...
- Completed: repaired the corrupted PBX references, added `BonjourTransport.swift` to the application target, added a real `FlashTeXCompanionTests` XCTest target, corrected an invalid SwiftUI accent-color call, and preserve validated encoded image data and MIME type through `CaptureStore` into `capture_submit`.
