...
- `FlashTeXMac` — the app.
- Tests (63, 2 gated): completion (prefix/trigger rules, unclosed `\end{}`, unsupported marks, non-ASCII
  and invalid carets, 1 MB latency) and navigation (label/ref incl. Unicode, nested begin/end,
  diagnostic cycling/wrap/refusal on the multipage sample, caret reveal); oversized complete line, trailing bytes at EOF, unsolicited/mismatched result correlation; inline diagnostic marks (byte→UTF-16, rebase/drop, path filter,
