...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        sender=None;events=(out/'events.jsonl').open('wb');sends=[];send_errors=[];acks={};previews=[]
    ...
    summary=dict(helper_sha256=sha(Path(args.helper).read_bytes()),producer_sha256=evidence['binary_sha256'],assets=evidence['assets'],
        sends=sends,acks=acks,previews=previews,final_revision=21,source_sha256=sha(states[-1].encode()),
    ...
    print(json.dumps(dict(ack_count=len(acks),preview_count=len(previews),preview_revisions=[p['revision'] for p in previews],post_last_send_preview_ms=summary['post_last_send_preview_ms'],send_ms=[s['sent_ms'] for s in sends])))
