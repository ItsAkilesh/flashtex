...
//! hand-written number, and three pairs had drifted onto the same one:
//! `Item::Table`/`Item::Logo` both 9, `Item::Footnote`/`Item::Rule` both 10,
//! and in `hash_math` `Nucleus::Rule`/`Nucleus::SubArray` both 15. The tag is
//! now `std::mem::discriminant` of the value, which is distinct for distinct
//! variants by construction, so a variant added later cannot silently reuse
//! another's tag.
//!
//! `no_two_item_kinds_share_a_key_stream` and
//! `no_two_math_nucleus_kinds_share_a_key_stream` are the regression tests
//! the task asks for: they fail if any two kinds ever map to the same value.
...
use flashtex_render_pipeline::adapter::{self, Item, Labels, Segment, TextStyle, Word};
...
fn one_of_every_item_kind() -> Vec<(&'static str, Item)> {
    ...
    vec![
        ...
        ("HFill", Item::HFill { fill: true }),
        ("HSpace", Item::HSpace { pt: 0.0 }),
        (
            "Table",
...
/// `Logo` both mapped to 9, `Footnote` and `Rule` both to 10 — and checking
/// whole keys would not have caught it, because their payloads differ in
/// length. It checks the tag itself.
#[test]
fn no_two_item_kinds_share_a_tag() {
    ...
    assert_eq!(seen.len(), 14, "a variant was added to Item without a sample here");
}
...
/// 107-item `Footnote`-plus-padding list absorb exactly the same bytes: on
/// the parent commit both hashed `0xcf4bedf5dbfa4a3b`. Neither the padding
/// (an `Item::Word` with no segments) nor a NUL `\@thefnmark` is anything
/// the adapter emits, so this was latent, not reachable — but it is what the
/// scheme permitted, and it must stay impossible.
#[test]
