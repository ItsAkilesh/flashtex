...
use flashtex_render_pipeline::display::{DisplayList, Item, Wire};
...
fn transform_fields_are_only_serialised_when_negotiated() {
    ...
    for (name, field, feature) in [("09-rotate-quarter.tex", "\"glyph_transform\":", "\"glyph_transform\""), ("16-trim-clip.tex", "\"clip\":", "\"image_clip\"")] {
        ...
        let negotiated = r.v2.write_json_wire("x", Wire { images: true, device_color: false, transforms: true });
        assert!(negotiated.contains(field) && negotiated.contains(feature), "{name}: {field} missing with display-list-v2-transforms");
        assert_eq!(negotiated, json::write(&r.v2.to_json_wire("x", Wire { images: true, device_color: false, transforms: true })), "{name}: direct writer differs from the value tree");
    }
}
