//! Float bodies gate: `tabular`, paragraphs, `center`, size declarations
//! and `\vspace` inside `figure`/`table` floats against the pdfLaTeX
//! (MacTeX 2026) references committed in `fixtures/float-tables/refs`
//! (made by `fixtures/float-tables/oracle.py`, test-only; cargo never runs
...
//! width and height; no `float_*` diagnostic.
...
#[test]
fn float_table_fixtures_match_pdflatex() {
    if !common::lm_available() {
        eprintln!("SKIP float_tables_oracle: Latin Modern fonts not installed");
        return;
    }
    let dir = concat!(env!("CARGO_MANIFEST_DIR"), "/fixtures/float-tables");
    let mut names: Vec<String> = std::fs::read_dir(dir)
        ...
        .collect();
    names.sort();
    assert!(names.len() >= 12, "expected at least 12 float-table fixtures, found {}", names.len());
