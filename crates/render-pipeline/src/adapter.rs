    ...
    for unit in split_at_page_breaks(texts, &lowered, size, &style, &parsed.theorems) {
        ...
        match unit.kind {
            ...
            UnitKind::Paragraph {
                ...
                let items = theorem_items(items_for(inlines, false), inlines.iter().map(inline_span).next(), theorem_head, &parsed.theorems, &parsed.qed_marks, &style);
                ...
                blocks.push(Block::Paragraph {
                    parts,
                    indent: !after_heading && !caption && styled.is_none() && !after_env && list.is_none() && !noindent && !theorem_head,
