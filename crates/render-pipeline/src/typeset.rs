...
pub fn build(ctx: &mut Context, doc: &Doc, cache: Option<&RenderCache>) -> Laid {
    ...
    let mut built = pagebuild::break_pages(&params, &list);
    ...
    let counters = geo.map_or_else(Vec::new, |g| {
        if columns == 1 && g.flags.twoside && !chapter_starts.is_empty() {
            open_right(&mut built, &chapter_starts, blocks.len(), &events, g.numbering);
        }
...
fn open_right(built: &mut Vec<pagebuild::BuiltPage>, chapter_starts: &[usize], n_blocks: usize, events: &[(usize, adapter::ChromeEvent, Span)], numbering: flashtex_class_geometry::Numbering) {
    let mut k = 0;
    while k < built.len() {
