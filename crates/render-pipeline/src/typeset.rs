    ...
    fn hlist(&mut self, items: &[AItem], size: f64, base: TextStyle, style: ParaStyle) -> (Vec<pl::Item>, Vec<Option<usize>>, Vec<(String, usize)>, Vec<(usize, f64)>) {
        ...
        for (idx, item) in items.iter().enumerate() {
            match item {
                ...
                AItem::Penalty { penalty } => push(&mut out, &mut recs, pl::Item::penalty(*penalty), None),
                AItem::Glue { em, stretch_em, shrink_em } => {
                    let quad = self.text_params(base, size).quad;
    ...
    fn paragraph_block(&mut self, items: &[AItem], indent: bool, starts_paragraph: bool, after_heading: bool, style: ParaStyle, list_geom: Option<&ListGeom>) -> Option<BuiltBlock> {
        ...
        if let Some(geom) = list_geom {
            ...
            if let Some(bib) = geom.bibliography.as_ref().filter(|_| starts_paragraph) {
                ...
                match geom.label.as_ref().and_then(|(text, span)| self.label_box(text, *span, size)) {
                    Some((run, rec)) => {
                        let w = run.width;
        ...
        let mut params = self.line_params(indent, self.style.baselineskip_pt, style, hang_pt);
