...
//! `\fontshape` + `\selectfont` for the text families the pipeline sets,
//! transcribed from MacTeX 2026 (`tex/latex/base/latex.ltx`, LaTeX
...
impl Scheme {
    ...
    pub fn for_document(packages: &[String], t1_encoding: bool) -> Scheme {
        let lmodern = packages.iter().any(|p| p == "lmodern");
