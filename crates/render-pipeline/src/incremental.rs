...
/// here. The hand-numbered constants this replaced had drifted into three
/// collisions — `Item::Table` and `Item::Logo` both hashed 9,
/// `Item::Footnote` and `Item::Rule` both hashed 10, and `Nucleus::Rule`
/// and `Nucleus::SubArray` both hashed 15 — which made two different item
/// lists hash to one key (see `tests/layout_cache_keys.rs`).
#[inline]
