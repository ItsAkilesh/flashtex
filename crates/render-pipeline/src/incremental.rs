...
pub fn block_origin(items: &[Item]) -> Option<(DocumentId, usize)> {
    ...
    for it in items {
        match it {
            ...
            Item::Table(_) | Item::ColorBox(_) | Item::Graphic(_) | Item::Transform(_) => return None,
...
pub fn hash_items(items: &[Item], base: usize, h: &mut DefaultHasher) {
    for it in items {
        match it {
            ...
            Item::EmGlue { em, stretch_em, shrink_em } => {
                30u8.hash(h);
                em.to_bits().hash(h);
            ...
            Item::SetLength { name, value } => {
                13u8.hash(h);
                format!("{name}{value:?}").hash(h);
            }
            Item::LengthGlue { dimen } => {
                14u8.hash(h);
                format!("{dimen:?}").hash(h);
            }
