...
pub fn handle_line(line: &str, fonts: &FontSet, options: &RenderOptions, cache: Option<&RenderCache>) -> Reply {
    ...
    if caps.display_list && v1.status != "failed" {
        ...
        let dl = if estimate > limit { None } else { Some(rendered.v2.write_json_wire(&id, crate::display::Wire { images: caps.images, device_color: caps.device_color, transforms: caps.transforms })) };
