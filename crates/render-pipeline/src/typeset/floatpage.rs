    ...
    loop {
        ...
        while i < nodes.len() {
            ...
            if let Some((h, d)) = bx {
                ...
                lines_seen += 1;
                prev_box = true;
                if total > is.goal - reserve + 1e-9 && lines_seen > 1 {
        ...
        if notes.iter().any(|l| !l.is_empty()) {
            // `\@makecol` with `\footins`, over a `\box\@cclv` the longtable
            // output routine may already have built: the body (here floats
            // as boxes, `\copy\LT@head` first and `\copy\LT@foot` last),
