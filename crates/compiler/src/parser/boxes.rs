...
impl P<'_> {
    ...
    fn box_blocks(&mut self, tokens: Vec<InputToken>) -> Vec<Block> {
        ...
        // The nested stream is owned here: edits to it are not logged against
        // the outer (possibly cache-lent) stream.
        let outer_undo = std::mem::take(&mut self.undo);
        let outer_lent = std::mem::replace(&mut self.lent_from_cache, false);
        ...
        // Macro scopes are handled by the expansion pass (`crate::expansion`).
        ...
        self.t = outer_tokens;
        self.undo = outer_undo;
        self.lent_from_cache = outer_lent;
        self.i = outer_index;
