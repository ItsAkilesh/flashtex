...
impl P<'_> {
    ...
    pub(super) fn tabular_environment(&mut self, open: Span, name: &str, para: &mut Vec<Inline>) {
        ...
        if features.longtable {
            ...
            self.current_counter = Some(self.table_counter.to_string());
        }
    ...
    fn table_command(
        ...
        let entry = match command {
            ...
            "caption" => {
                ...
                Entry::Caption {
                    content,
                    number: (!starred).then_some(self.table_counter),
            ...
            _ => {
                let color = self.color_argument(command, span);
    ...
    /// A colour argument `[model]{spec}`.
    pub(super) fn color_argument(&mut self, command: &str, span: Span) -> ColorSpec {
        let model = self
    ...
    fn strip_cell_commands(
        ...
        loop {
            ...
            if is_color {
                let (c, unread) = self.with_tokens(tail, |p| p.color_argument("cellcolor", span));
                color = Some(c);
                rest = unread;
            } else {
