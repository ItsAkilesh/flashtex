...
impl Counters {
    ...
    pub fn report() -> Self {
        ...
        counters.number_within("subsubsection", "subsection");
        counters.define_body_counters();
        // report.cls/book.cls `\@addtoreset{equation|figure|table}{chapter}`
        // with `\the<counter>` = `\thechapter.\@arabic` (integration
        // 2026-09-13b: #153 kept these by hand, #155 moved them here).
        for name in ["equation", "figure", "table"] {
            let _ = counters.counter_within(name, "chapter", false);
        }
        counters
    }
    ...
    pub fn set_within(&mut self, name: &str, parent: &str) -> bool {
        ...
        // #155 keeps every `\@addtoreset` parent (`reset_by` is a list) and
        // `\the<name>` as pieces; refuse a cycle through any parent chain.
        let mut pending = vec![parent_index];
        let mut visited = 0;
        while let Some(at) = pending.pop() {
            ...
            visited += 1;
            if visited > self.counters.len() * self.counters.len() {
                break;
            }
            pending.extend(self.counters[at].reset_by.iter().copied());
        }
        self.add_to_reset(index, parent_index);
        self.counters[index].the = within_pieces(name, parent, NumberStyle::Arabic);
        true
    }
