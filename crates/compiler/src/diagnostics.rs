...
mod tests {
    ...
    // ── to_json / to_json_with_paths: shape ───────────────────────────────────

    #[test]
    fn to_json_has_required_top_level_keys() {
        let v = error_with_span().to_json("doc.tex");
    ...
    // ── source span: multi-document via to_json_with_paths ────────────────────

    #[test]
    fn to_json_with_paths_picks_correct_path_for_document() {
        // Span in document 1 should resolve to paths[1].
        let span = Span::in_document(DocumentId(1), 0, 5);
        let d = Diagnostic::error("msg", Some(span), None);
        let paths = &["main.tex", "included.tex"];
        let v = d.to_json_with_paths(paths);
        let src = v.get("source").unwrap();
        assert_eq!(src.get("path").and_then(|s| s.as_str()), Some("included.tex"));
    }

    #[test]
    fn to_json_with_paths_falls_back_to_empty_string_for_out_of_range_document() {
        // Document index beyond paths slice → empty string path.
        let span = Span::in_document(DocumentId(99), 0, 1);
        let d = Diagnostic::error("msg", Some(span), None);
        let v = d.to_json_with_paths(&["only.tex"]);
        let src = v.get("source").unwrap();
        assert_eq!(src.get("path").and_then(|s| s.as_str()), Some(""));
    }
