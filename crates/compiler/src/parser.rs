...
impl P<'_> {
    ...
    fn command(&mut self, name: &str, span: Span, blocks: &mut Vec<Block>, para: &mut Vec<Inline>) {
        ...
        match name {
            ...
            // `\numberwithin` is #155's `counter_numbering` (with `[style]`),
            // dispatched below with `\counterwithin`/`\counterwithout`.
            ...
            "caption" => {
                ...
                if matches!(float.as_str(), "algorithm" | "algorithm*") {
                    ...
                    self.flush_paragraph(blocks, para);
                    let (counter, caption_name) = if float == "table" {
                        ("table", "Table")
                    } else {
                        ("figure", "Figure")
                    };
                    let number_text = self.counters.step(counter).unwrap_or_default();
                    self.current_counter = Some(number_text.clone());
                    let number = self.counters.value(counter).unwrap_or(0);
    ...
    fn set_length(&mut self, span: Span, para: &mut Vec<Inline>) {
        ...
        let target = token_text(&target_tokens)
            ...
            .trim_start_matches('\\')
            .to_string();
        // `\fboxsep`/`\fboxrule` feed both #137's box lengths (the
        // `Inline::SetLength` below) and #150's `\colorbox` capture, so record
        // them before `set_box_length` claims the assignment.
        if matches!(target.as_str(), "fboxsep" | "fboxrule") {
            let body = self.class_size_pt.unwrap_or(crate::layout::BODY_SIZE_PT);
            if let Some(pt) = parse_dimen_pt_at(&token_text(&value_tokens), body) {
                if target == "fboxsep" {
                    self.fboxsep_pt = pt;
                } else {
                    self.fboxrule_pt = pt;
    ...
    fn begin_item(&mut self, span: Span, explicit: Option<(Vec<InputToken>, Span)>) {
        ...
        let enclosing_label = self
            ...
            .unwrap_or_default();
        // hyperref (#131): a counted enumerate `\item` is `\refstepcounter
        // {enum<i>}` and gets an `Item.<n>` anchor; `\item[..]` is neither.
        let mut hyperref_item: Option<Option<String>> = None;
        ...
        let item = match explicit {
            ...
            None if environment == ListEnvironment::Enumerate => {
                ...
                let templated = list.label_star.is_some() || list.template.is_some();
                hyperref_item = Some(templated.then(|| list.current_label.clone()));
                item
            }
        ...
        if let Some(templated_marker) = hyperref_item {
            self.enumerate_item_hyperref(templated_marker, span);
        }
    }
    ...
    fn counter_numbering(&mut self, name: &str, span: Span) {
        ...
        self.document_global_state = true;
        // Theorem counters are ordinary `xref::Counters` entries since #149
        // (`\newtheorem` defines them), so they take the general path.
