...
/// from; `ex` uses the common TeX-metrics approximation of half an em, since
/// no real x-height is read from the font. `bp` ("big point") is exactly this
/// compiler's own internal point (both are 1/72 inch, matching the 612×792pt
/// page in `layout.rs`), unlike `in`/`cm`/`mm` below, which follow TeX's own
