...
def audit():
 ...
 assert len(out['payload']['diagnostics'])==e['diagnostics']==103 and not(new-old)
 removed=[json.loads(s) for s,n in (old-new).items() for _ in range(n)];assert len(removed)==16
 spans=[];counts=collections.Counter()
 for page in out['payload']['pages']:
  for item in page['items']:
   if item.get('text') not in ['∀','∃']:continue
   span=item['source'];symbol=item['text'];token={'∀':b'\\forall','∃':b'\\exists'}[symbol]
 ...
 assert all(d['severity']=='error' and any(cmd in d['message'] for cmd in ['\\forall','\\exists']) for d in removed)
 return {'peer':PEER,'binary_sha256':e['binary_sha256'],'source_sha256':sha(source),'file_hashes':e['files'],'diagnostics':[119,103],'removed':removed,'symbol_counts':dict(counts),'spans':spans,'scope':'exact archived request/output and preserved executable checked; no rerun/PDF/native/combined adoption'}
