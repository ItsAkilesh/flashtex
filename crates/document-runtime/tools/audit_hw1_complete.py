...
def audit():
 e=json.loads(read('hw1-quantifiers/combined-evidence.json'))
 for n,h in e['files'].items():assert sha(read('hw1-quantifiers/'+n))==h
 assert sha(Path(e['binary_path']).read_bytes())==e['binary_sha256']
 req=json.loads(read('hw1-quantifiers/combined-request.jsonl'));out=json.loads(read('hw1-quantifiers/combined-response.jsonl'))
 ...
 assert len(out['payload']['diagnostics'])==e['diagnostics']==84
 removed=[json.loads(s) for s,n in (old-new).items() for _ in range(n)];assert len(removed)==49
 ...
 for page in out['payload']['pages']:
  for item in page['items']:
   ...
   span=item['source'];symbol=item['text'];token={'∀':b'\\forall','∃':b'\\exists','∈':b'\\in'}[symbol]
   assert span['path']=='HW1.tex' and source[span['start_byte']:span['end_byte']]==token
   counts[symbol]+=1;spans.append(span)
 assert counts=={'∀':10,'∃':6,'∈':12}
 symbol_removed=[d for d in removed if d['message'] in ['\\forall is not supported in math mode','\\exists is not supported in math mode','\\in is not supported in math mode']]
 assert len(symbol_removed)==28
 assert collections.Counter(map(key,spans))==collections.Counter(key(d['source']) for d in symbol_removed)
 return {'peer':PEER,'binary_sha256':e['binary_sha256'],'source_sha256':sha(source),'file_hashes':e['files'],'diagnostics':[119,84],'added':added,'removed':removed,'symbol_counts':dict(counts),'spans':spans,'scope':'exact archived request/output and preserved executable checked; no rerun/PDF/native/adoption'}
