...
use flashtex_font_engine::{Face, FontSource, GlyphId, KerningSource, TrueTypeFace, load_from_path};
...
fn times_new_roman_metrics_and_identity() {
    ...
    assert!((1300..=1400).contains(&vm.cap_height), "cap_height {}", vm.cap_height);
}
...
fn av_kerning_via_gpos_or_kern_is_negative() {
    ...
    assert!(matches!(src, KerningSource::Gpos | KerningSource::KernTable));
    let kerned = shape(&f, "AV", &ShapeOptions::default()).unwrap();
    let plain = shape(&f, "AV", &ShapeOptions::PLAIN).unwrap();
    assert_eq!(kerned.advance_units(), plain.advance_units() + i64::from(adj));
...
fn fi_ligature_from_gsub_or_cmap() {
    ...
    let off = shape(&f, "fi", &ShapeOptions { ligatures: false, ..ShapeOptions::default() }).unwrap();
    assert_eq!(off.clusters.len(), 2);
    assert_eq!(on.clusters.len(), 1, "expected a ligature (GSUB liga or cmap U+FB01)");
...
fn combining_mark_composes_via_cmap() {
    ...
    assert_eq!(composed.clusters[0].glyphs[0].gid, pre.clusters[0].glyphs[0].gid);
...
fn unicode_repertoire_in_arial_unicode() {
    ...
    assert_eq!(i64::from(zhong.advance), i64::from(f.units_per_em()), "ideograph is 1 em");
}
...
fn missing_glyph_in_truetype_is_notdef_and_listed() {
    ...
    assert_eq!(s.clusters[1].glyphs[0].advance, i32::from(f.advance(GlyphId::NOTDEF).unwrap()));
}
...
fn subsetting_is_deterministic_and_checksummed() {
    ...
    for (&old, &new) in &a.old_to_new {
        assert_eq!(sub.advance(GlyphId(new)).unwrap(), f.advance(GlyphId(old)).unwrap());
    }
...
fn to_unicode_round_trip_including_ligature_text() {
    ...
    for cluster in &shaped.clusters {
        for g in &cluster.glyphs {
            ...
            assert!(usize::from(cid) < pdf.widths.len());
        }
    }
    ...
    assert_eq!(pdf.widths[0], f.to_pdf_units(i64::from(f.advance(GlyphId::NOTDEF).unwrap())));
...
fn bounded_resolution_never_scans() {
    let search = FontSearch::new().with_dir("/nonexistent/dir").with_macos_system_dirs();
    ...
    if Path::new(TIMES_NEW_ROMAN).is_file() {
        assert_eq!(search.find("Times New Roman.ttf").unwrap(), Path::new(TIMES_NEW_ROMAN));
...
fn malformed_and_unsupported_inputs_are_errors_not_panics() {
    ...
    assert!(matches!(
        TrueTypeFace::parse(otto),
        Err(flashtex_font_engine::Error::Unsupported(_))
    ));
}
