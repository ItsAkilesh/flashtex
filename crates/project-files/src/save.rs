...
/// The same identity for an entry that was classified by name with
/// [`sys::stat_at_nofollow`] rather than opened.
///
/// It is deliberately the *same* value the open handle yields — POSIX
/// `(st_dev, st_ino)` from `fstatat`, Windows' `FILE_ID_INFO` pair from the
/// attributes-only open — so a `FileIdentity` taken from a descriptor and one
/// taken from a name can be compared, which is the whole of [`TempFile`]'s
/// and [`verify_installed`]'s check.
#[cfg(unix)]
fn stat_identity(st: &sys::EntryStat) -> FileIdentity {
    FileIdentity {
        dev: st.dev,
        ino: st.ino,
    }
...
fn stat_identity(st: &sys::EntryStat) -> FileIdentity {
    FileIdentity {
        volume_serial: st.volume_serial,
        file_index: st.file_index,
    }
}

#[cfg(not(any(unix, windows)))]
fn stat_identity(_: &sys::EntryStat) -> FileIdentity {
    // Unreachable: `sys::stat_at_nofollow` is `Unsupported` here, and
    // `ProjectRoot::open` refuses before anything is stat'ed.
    FileIdentity { _unsupported: () }
}

/// The permission bits a newly created temp file gets when there is no
/// existing file to copy them from. Ignored where there are none.
...
impl ProjectRoot {
    ...
    /// pinned (same file, not a symlink). `Ok(false)` when the path is gone
    /// or now names something else: the root was renamed, removed or
    /// replaced after it was opened. Ancestors of the path are resolved
    /// normally, as in [`Self::open`].
    ///
    /// The comparison re-opens the path with [`sys::open_dir_nofollow`] —
    /// the same no-follow open [`Self::open`] used — and compares
    /// [`FileIdentity`] values, rather than reading `symlink_metadata`:
    /// Windows exposes no file ID through `std::fs::Metadata` on stable
    /// Rust, so identity has to come from a handle (see [`identity`]). The
    /// three ways the path can fail to be this directory any more — absent,
    /// not a directory, a symlink — are the three refusals that open
    /// already reports, and each is `Ok(false)`.
    pub fn is_still_at_path(&self) -> io::Result<bool> {
        let pinned = identity(&self.dir)?;
        match sys::open_dir_nofollow(&self.path) {
            Ok(dir) => Ok(identity(&dir)? == pinned),
            Err(e)
                if e.kind() == io::ErrorKind::NotFound
                    || sys::errno_is(&e, sys::ENOENT)
                    || sys::errno_is(&e, sys::ENOTDIR)
                    || sys::errno_is(&e, sys::ELOOP) =>
            {
                Ok(false)
            }
            Err(e) => Err(e),
...
impl ProjectLock<'_> {
    ...
    pub(crate) fn save_with(
        ...
        // `None` both where the platform has no permission bits and where
        // there is no existing file to copy them from; either way the temp
        // file keeps what `O_CREAT` gave it.
        let mode = before.as_ref().and_then(|o| o.mode);
        let flags = sys::O_WRONLY | sys::O_CREAT | sys::O_EXCL | sys::O_NOFOLLOW;
        let mut temp = sys::open_at(&dir, &temp_name, flags, mode.unwrap_or(DEFAULT_MODE))
        ...
        let temp_identity = match identity(&temp) {
            Ok(id) => id,
            Err(e) => {
        ...
        if let Err(e) = (|| -> io::Result<()> {
            ...
            temp.sync_all()?;
            // A no-op when `mode` is `None`, which covers both "no existing
            // file to copy from" and "this platform has no permission bits".
...
fn replace_target(
    ...
    if !force {
        let unchanged = match (recheck, current) {
            (None, None) => true,
            (Some(o), Some(st)) => o.identity == stat_identity(&st),
            _ => false,
        };
        if !unchanged {
...
impl TempFile<'_> {
    ...
    fn still_ours(&self) -> io::Result<bool> {
        match sys::stat_at_nofollow(self.dir, self.name.as_bytes()) {
            Ok(st) => Ok(st.is_file() && stat_identity(&st) == self.identity),
            Err(e) if sys::errno_is(&e, sys::ENOENT) => Ok(false),
            Err(e) => Err(e),
        }
    }
...
fn verify_installed(
    ...
    let installed = match sys::stat_at_nofollow(dir, name.as_bytes()) {
        Ok(st) => st.is_file() && stat_identity(&st) == written,
...
mod tests {
    ...
    ///
    /// Unix only, because the hard-link install is: Windows always has a
    /// no-replace rename, so [`sys::link_at`] reports `ENOTSUP` there and
    /// this configuration is the one
    /// `new_file_without_any_noclobber_primitive_is_refused_unless_forced`
    /// covers instead. (`nlink` is also unix-only.)
    #[cfg(unix)]
    #[test]
    ...
    /// Used only by the temp-name swap cases, which need a hard link or a
    /// symlink to plant the swap and are therefore unix-only.
    #[cfg(unix)]
