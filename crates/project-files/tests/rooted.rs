...
use std::fs;
// `FileTypeExt` is only needed by the FIFO and socket cases, which have no
// Windows equivalent and are `#[cfg(unix)]` themselves; `MetadataExt` and
// `symlink` are not used at all, because link creation goes through
// `common::symlink_dir`/`symlink_file`, which pick the platform's own call.
#[cfg(unix)]
use std::os::unix::fs::FileTypeExt;
use std::sync::Arc;
use std::sync::atomic::{AtomicBool, Ordering};
// Only the FIFO cases need a watchdog thread, and only unix has FIFOs.
#[cfg(unix)]
use std::sync::mpsc;
use std::thread;
#[cfg(unix)]
...
#[cfg(unix)]
use flashtex_project_files::{DiagnosticKind, DiscoverError, ProjectGraph};

/// Removes a *directory* symlink, leaving its target alone.
///
/// POSIX unlinks a symlink of any kind with `unlink`, but Windows routes a
/// directory symlink through `RemoveDirectoryW` (`fs::remove_dir`) and
/// rejects it in `DeleteFileW` (`fs::remove_file`) with `ERROR_ACCESS_DENIED`.
/// Neither call follows the link.
fn remove_symlink_dir(link: &std::path::Path) {
    #[cfg(unix)]
...
/// Windows has no FIFO, and no other file type that blocks on open, so the
/// whole family of "a special file must be refused without blocking" cases
/// below is unix-only. The behaviour they protect — classify before open,
/// never block — is exercised on Windows by the directory and reparse-point
/// cases, which are the special entries that exist there.
#[cfg(unix)]
...
fn save_rename_window_only_replaces_the_directory_entry() {
    for force in [false, true] {
        ...
        let (guard, fired) = swap_once_at(Window::BeforeRename, "a.tex", move || {
            ...
            symlink_file(&victim2, &link_target);
        });
...
fn new_file_install_window_never_replaces_a_planted_symlink() {
    for force in [false, true] {
        ...
        let (guard, fired) = swap_once_at(Window::BeforeRename, "new.tex", move || {
            symlink_file(std::path::Path::new("elsewhere.tex"), &plant);
        });
...
fn watcher_never_hashes_through_a_symlinked_tracked_file() {
    ...
    fs::remove_file(t.root().join("a.tex")).unwrap();
    symlink_file(&victim, &t.root().join("a.tex"));
    let diff = snap.diff().unwrap();
...
/// the test has nothing to exercise and says so. Windows file names are
/// UTF-16, so a non-UTF-8 one cannot be constructed there at all.
#[cfg(unix)]
#[test]
...
fn remove_refuses_special_files_and_broken_symlinks() {
    ...
    make_fifo(&t.root().join("pipe.tex"));
    symlink_file(&t.root().join("nowhere.tex"), &t.root().join("broken.tex"));
    t.write("plain.tex", "x");
    let root = ProjectRoot::open(t.root()).unwrap();
...
fn recovery_listing_refuses_a_symlinked_journal_directory() {
    ...
    symlink_dir(outside.root(), &t.root().join(".flashtex"));
    let root = ProjectRoot::open(t.root()).unwrap();
